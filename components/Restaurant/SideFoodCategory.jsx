...
function SideFoodCategory({restaurants}) {
  ...
  console.log("section1InView",section1InView,"section2InView",section2InView)
  return (
...

        {
          restaurant.restCatsMenu.map((cat)=>{
            if(cat.name=="فودپارتی"){
              return (
                <p ref={section1Ref} id={cat.id} key={cat.id} onClick={selectCatHandler} className={clsx('mb-4 py-0.5 px-3 text-sm cursor-pointer font-iransans inline-block ',selectCat==cat.id ? "border-r-2 border-r-carbon-main laptop:border-l-carbon-main  laptop:border-l-2 laptop:border-r-0 font-bold text-carbon-main" : "text-carbon-light")}>
              ...
              return (
                <p ref={section2Ref} id={cat.id} key={cat.id} onClick={selectCatHandler} className={clsx('mb-4 py-0.5 px-3 text-sm cursor-pointer font-iransans inline-block ',selectCat==cat.id ? "border-r-2 border-r-carbon-main laptop:border-l-carbon-main  laptop:border-l-2 laptop:border-r-0 font-bold text-carbon-main" : "text-carbon-light")}>
